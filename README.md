...
**Compute sqrt of k [√]** - without sqrt() in math.h compute sqrt with Newton-method.
...
**Connected Graph [√]** - Create a program which takes a graph as an input and
    outputs whether every node is connected or not. Use DFT to count connecting
    components. 
...
**Remove mid node [√]** - Implement an algorithm to delete a node in the middle of a single linked list, given only access to that node.(CC 150)
...
**Rotate AB string to BA [√]** - Enter a string and the rotation position *i* will reverse it and print it out.
...
**unique characters [√]** - Implement an algorithm town determine if a string has all unique characters. What if you can not use additional data structures?(CC150)


**remove duplicate [√]** - Design an algorithm and write code to remove the duplicate characters in a string without using any additional buffer. NOTE: One or two additional variables are fine. An extra copy of the array is not.
FOLLOW UP
...
**isRotation [√]** - Assume you have a method isSubstring which checks if one
    word is a substring of another. Given two strings, s1 and s2, write code to
    check if s2 is a rotation of s1 using only one call to isSubstring i.e.,
    “waterbottle” is a rotation of “erbottlewat”.(CC 150)
...
**Trutle polygon and circle [√]** - draw polygon and circle with a turtle.(Think
    Python Ch4)
